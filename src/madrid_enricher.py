...
SYSTEM_PROMPT = (
  ...
  "- Numbers must be plain numbers (no commas), URLs include scheme.\n"
)
...
def _coerce_types(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k in TARGET_COLS:
        v = d.get(k)
        out[k] = _safe_number(v) if k in NUMERIC else (None if v is None else str(v).strip())
    return out
...
def _update_row(row_id: str, patch: Dict[str, Any]) -> None:
    ...
    UPDATE {TABLE_FQN}
    SET {', '.join(sets)}
    WHERE CONCAT(COALESCE(LOWER(name),''),'|',COALESCE(LOWER(domain),'')) = @id
    """
    bq.query(sql, job_config=bigquery.QueryJobConfig(
        query_parameters=[bigquery.ScalarQueryParameter("id","STRING", row_id), *params]
    )).result()

def _fetch_batch(limit: int) -> List[Dict[str, Any]]:
    sql = f"""
    SELECT
      CONCAT(COALESCE(LOWER(name),''),'|',COALESCE(LOWER(domain),'')) AS _id,
    ...
    LIMIT @lim
    """
    return [dict(r) for r in bq.query(sql, job_config=bigquery.QueryJobConfig(
...
def _enrich_one(rec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if txt.startswith("```"):
        txt = txt.strip("`")
        if "\n" in txt: txt = "\n".join(txt.split("\n")[1:])
    return _coerce_types(json.loads(txt))

@app.get("/ping")
def ping(): return "pong"

@app.get("/__routes")
def routes(): return jsonify(["/__routes","/ping","/probe/bq","/probe/openai","/enrich_all_nulls"])
...
def probe_openai():
    try:
        if not oai: return jsonify({"ok": False, "error": "OPENAI_API_KEY missing"}), 500
        r = oai.chat.completions.create(model=MODEL, messages=[{"role":"user","content":"ok"}], temperature=0)
        return jsonify({"ok": r.choices[0].message.content.strip().lower()=="ok"})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500

@app.post("/enrich_all_nulls")
def enrich():
    if not oai: return jsonify({"status":"error","error":"OPENAI_API_KEY missing"}), 500
    try:
    ...
    for b in range(max_batches):
        ...
        if not rows:
            return jsonify({"status":"done","updated":total,"batches":b})
        for r in rows:
            rid = r["_id"]
            try:
                patch = _enrich_one(r)
            except HTTPStatusError as he:
                if getattr(he, "response", None) and he.response.status_code == 429:
                    return jsonify({"status":"stopped_on_rate_limit","updated":total})
                continue
            except Exception:
                continue
            try:
                _update_row(rid, patch)
                total += 1
            except Exception:
                pass
            time.sleep(sleep)
    return jsonify({"status":"stopped_on_max_batches","updated":total,"batches":max_batches})
